...
<template>
    <div class="heat-map" :class="{'wx-heat-map':wxHeatMap,'isandrion':isAndrion,'andcss':Andrion}">
         <div class="type-search" :class="{'wx-type-search':wxHeatMap}">
                <flexbox :class="{isnotLinerent:isonLinerent}">
                    <flexbox-item>
                        <div class="flex-demo" @click="regionClick">
                                ...
                                <img class="open-icon" v-if="regionClass" :src="openUrl" :class="{isnewhouse:isnewhouse}"/>
                                <img class="stop-icon" v-if="!regionClass" :src="stopUrl" :class="{isnewhouse:isnewhouse}"/>
                                <span class="long-string" :class="{notnewhouse:isonLinerent}"></span>
                        </div>
                    </flexbox-item>
                    <flexbox-item>
                        <div class="flex-demo" @click="subwayClick">
                                <span class="regionName" :class="{regionarea:subwayClass}">{{subwayNames}}</span>
        ...
        <!-- 筛选 -->
        <div  v-show="filtrate" class="regloinlist">
            <filtrate :isShow="filtrate" @changeFiltrate="changeFiltrate" :houseType="houseType" :isMap="isMap" ref="filtrateCommpont"></filtrate>
...
// import MyBus from '../../script/myBus.js';
    ...
    import icon_ditu2 from '../../../../static/rent/fyxq/icon/ditu2.png'
    export default {
        components: {
            Flexbox,
            FlexboxItem,
        ...
        created(){
            ...
            this.getAddressComponent(this.$route.query.title);
        ...
        methods: {
            ...
            //调用微信接口
        	// wxconfig() {
            //     return this.post("jsapi/getJsapiSignature?local_url=" + encodeURIComponent(location.href),
            //         {},{
            //             interfaceType: "weichat"
            //         }).then(response => {
            //         // console.log('初始化微信,配置config');
            //         this.$wechat.config({
            ...
            //         success:  (res1)=> {
            //             this.longitude = res1.longitude; //纬度
            //             this.latitude = res1.latitude;
            //             console.log(res1.longitude,res1.latitude,res1)
            //             this.getwexmap(data);
            //         }
            //     });
            // },
                ...
                if(this.isECTouch()){
                    if(this.platform){
                        ...
                        // 浏览器支持geolocation
                            ...
                            geolocation.getCurrentPosition(result => {
                                ...
                                // mk.disableMassClear();
                                // if(data=="地铁找房"){
                                //     this.getLocationSubwayLine();
                                // }else{
                                //         // 创建标注
                                //     // this.map.centerAndZoom(new BMap.Point(this.longitude,this.latitude),14);
                                //     this.circle = new BMap.Circle(point,2000,{fillColor:"#999", strokeWeight: 1 ,fillOpacity: 0.3, strokeOpacity: 0.3}); //创建圆
                    ...
                    baidumap_location.getCurrentPosition( (result)=> {
                        if(this.$route.query.isSubway){
                            ...
                            this.send(result.longitude, result.latitude)
                        }
            ...
            getLocationSubwayLine(){
                this.regionSearch = false;
                let subwaylineList = "";
                let subwayType = {name:"",subway:"",subwayLineId:""};
                this.local1 = new BMap.LocalSearch(this.map, {
                    onSearchComplete:(results)=>{
                        // this._getContent(results.Br)
                        this.getRegionSubway(true,()=>{
                            this.subwayList.forEach(item=>{
                                if(results.Br[0].address.split(";")[0].indexOf(item.name)!==-1){
                                    subwayType.subwayLineId = item.id;
                                    this.post("common/getStationLine",{
                                        subwayId: item.id
                                    }).then((res)=>{
                                        subwaylineList = res.data;
                                    }).then(res=>{

                                        this.post("pc/HouseInfoController/getSubwayLineHouseCount",{ //地图里展示站点套数
                                            subwayLineId: item.id,
                                        }).then(res1=>{
                                            subwaylineList = subwaylineList.map((item1,index)=>{
                                                for(var i = 0;i < res1.data.length;i++){
                                                    if(res1.data[i].stationsOne === item1.id){
                                                        item1.totalNum = res1.data[i].totalNum;
                                                    }
                                                }
                                                return item1;
                                            })
                                        }).then(()=>{
                                            this.getSubwayMap(item.name,subwaylineList,subwayType);
                                        })
                                    })
                ...
                this.local1.searchNearby("地铁线路",new BMap.Point(this.longitude, this.latitude));
            },
            send(long, lat){
                this.longitude = long; //纬度
                this.latitude = lat; //使用百度地图API //创建地图实例
                var point = new BMap.Point(this.longitude, this.latitude);
                var myIcon = new BMap.Icon(icon_ditu2,
                new BMap.Size(45,70));
            ...
            changeRegionSubway(id,regionId,name){
                this.dynamicTags.districtId = id.isAreaTypeid || "";    //区域ID
                ...
                if(id && !!name===false){
                    this.area.forEach(item=>{
                        if(item._id === id.isAreaTypeid){

                            this.oneregionSearch = false;   //点击区域的时候不进行缩放事件
                            this.isMoveend = false;    //点击区域的时候不可以拖拽加载
                            this.areaId = id.isAreaTypeid;
                            this.dynamicTags.districtId = id.isAreaTypeid;
            ...
            //地铁返回数据
            changeSubway(name,subwayList,subwayType,nametwo){
                //点击地铁区域置空
                ...
                if(name){
                    ...
                    this.dynamicTags.subwayLineId = subwayType.subwayLineId || "";
                     if(subwayType.subwayId){
                        this.subwayNames = nametwo;
                    }else{
                        this.subwayNames = name;
                    }
