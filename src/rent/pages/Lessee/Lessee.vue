...
<template>
    <div class="Lessee-box fixContent">
        ...
        <div class="Lessee-box-center">
            ...
            <div class="Lessee-box-center-jy">
                <div>
                     <p class="sign-sign-p1">变更租约</p>
                    <span class="sign-sign-p2" @click="surrendClick">{{leasetype}}</span>
                </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.post('RentContractController/getRentContractDetail',{
                ...
                if(res.data.picImage){
                    ...
                    this.picImage = "https://img.guoanfamily.com/rent/static/HomePage/noDetailImg.png";
                }
                ...
                if(res.data.unRentId){
                    this.unRentId = res.data.unRentId;
                    this.leasetype = "退租详情"
                }else{
                    this.leasetype = "办理退租"
                }
        ...
        methods: {
            surrendClick(){
                if(this.unRentId){
                    this.$router.push({path:"/surrenderDetail",query:{id:this.unRentId}})
                }else{
                    this.$router.push({path:"/applySurrender",query:{id:this.saleContractId}})
                }
            ...
            lease(){
            	//2018-11-27.加上判断，判断当前支付状态，如果是已支付，点击则不能跳转到支付页面，若是待支付则点击可以跳转到支付页面。
            	if(this.payStatus == "待支付"){
            		let platformnow = localStorage.getItem("platformnow")
					if(platformnow=="xiaochengxu"){
						let token = localStorage.getItem("token")
						this.$wechat.miniProgram.navigateTo({url:`/pages/contractPay/contractPay?receiptPlanId=${this.receiptPlanId}&saleContractId=${this.saleContractId}&token=${token}`})
					}else{
