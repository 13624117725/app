				...
				.tuijian-li{
					...
					.tuijian-li-img{
						...
						.tuijian-li-money{
							...
							top: 0.2rem;
							left: 0.2rem;
							padding:0.1rem 0.18rem 0.1rem 0.15rem;
...
<template>
	<div class="fixContent">
		...
		<div class="loadingBox" v-show="AppShear">
			<div class="loadings">
				<div class="loadings_img">
					<img src="../../../../static/new/img/shear1.png" alt=""/>
				</div>
				<div>
				分享中...
				</div>
		...
		<div class="page-content" id="pageBody">
			<div :class="[this.isIosandroid() ? 'emptyandroid' : 'emptyios']" v-if="!this.$store.state.showWxTitle"></div>


			<div style="position: relative;height: 5rem;padding-top: 0.55rem;">
				<!--弹幕-->
                ...
                <!--收藏分享-->
                <div :class="!isBanShare ? 'collectBtnWx' : 'collectBtn'" @click="collectFunc" :style=" isCollect ? collectYes : collectNo ">
				...
				<div class="collectBtn shareBtn" @click="buildShare" v-if="isBanShare">
				...
				<swiper
			        ...
			        <swiper-slide v-for="(item,index) in imgUrl" :key = "index">
			        	<!--3D展示按钮-->
						<div class="threeShow" v-if="index == 0 && lookUrl" @click="goToShow($event)"><img src="https://img.guoanfamily.com/www/3dlogos.png" alt="" /></div>
			        	<img :src="item.img" style="width: 100%;" alt="" />

			        	<!--品质节logo-->
		            	<!--<img style="width: 2.2rem;height: 0.7rem; position: absolute;bottom: 0.1rem;right: 0.3rem;z-index: 10;" src="https://img.guoanfamily.com/rent/pinzhijie/pinzhiLogo.png" alt="" />-->
			...
			<!--房间信息与收藏-->
			<div class="collect">
				<p style="font-size: 0.42rem;text-align: left;text-indent: 0.1rem;">{{areaName}} <span v-if="isRoomOrHouse">{{houseName}}</span> {{houseName == '东大桥店'?roomFloor:colledtName}}{{roomNumber}}</p>

				<div class="collect-title">
					<div class="priceShow">{{Price == null?0:Price}}</div>
					<div class="priceShowMonth">元/月</div>
				</div>
			...
			<div class="houseRoomDetail">
				...
				<div class="facility">
					<div class="peitao-box">
						<ul class="facility-title-slide">
							<li
								class="facility-item"
								...
								<p v-if="item.typeCode == '0014001'">{{item.roomItemName+roomNumber+'房间'}}</p>
				...
				<div v-if="isHaveroomList">
					<div class="houseRoomDetail-title" style="padding-bottom: 0.05rem;">
						<span>室友信息</span>
					</div>
					<div class="roommate">
				...
				<div>
					<div class="houseRoomDetail-title" style="padding-top: 0.2rem;">
						<span>周边</span>
					</div>
					<div style="color: #666666;font-size: 0.28rem;line-height:0.5rem; text-align: left;box-sizing: border-box;padding-left: 0.1rem;">
						{{surrounding}}
					</div>
				</div>
				<div>
				...
				<div v-if="isHaveRecommend">
					...
					<div class="community-img" v-if="isHaveCommunityImages">
			    		<ul class="community-ul">
			    			<li class="community-li" v-for="(itemImg,indexCommunity) in communityList" :key="indexCommunity">
			    				<img :src="itemImg"/>
			    			</li>
			    		</ul>
			    	</div>
					<div class="community">
						<div v-if="isHaveCommunityImages" style="height: 0.12rem; border-top: 0.01rem solid #f2f2f2;width: 98%;"></div>
						<div style="padding:0 0.2rem 0 0.1rem;text-align: left;line-height:0.5rem;" v-if="isHaveRecommend"><p style="color: #666666;">{{recommend}}</p></div>

					</div>
				</div>
			</div>
			<!--国安家推荐-->
			<div class="recommend" v-if="isHaveGaj">
				<div style="height: 0.2rem;background: #EEEEEE;"></div>
				<div class="houseRoomDetail-title" style="text-align: left;padding-left: 0.4rem;margin-top: 0.3rem;">
					<span >国安家推荐</span>
				</div>
				<div class="ga-recommend" >
						...
						<ul class="tuijian-slide">
							<li class="tuijian-li" v-for="(item,index) in recommendList" :key="index" @click="recommendDetali(item)">
								<div class="tuijian-li-img">
									<img :src="item.image" />
									<!--品质节logo-->
		            				<!--<img style="width: 1.3rem;height: 0.5rem; position: absolute;bottom: 0.1rem;right: 0.1rem;z-index: 2;" src="https://img.guoanfamily.com/rent/pinzhijie/pinzhiLogo.png" alt="" />-->
				...
				<div style="height: 1rem;background: white;"></div>
			</div>
...
//		           	},
		...
		methods: {
			...
			//随机弹幕
			getDanmu(){
				this.post("/HouseBarrageController/getHouseBarrageRandomList").then((res)=>{

					for(var i=0;i<res.data.length;i++){
						this.barList[i].text = res.data[i].barrage;
					}
				})
			},
			//房屋配套点击事件
			clickFacility(item,index){
				...
				for(let i=0;i<this.roomItems.length;i++){
					if(i== index){
						...
						facilityItemTitle[i].style.borderBottom="0"
					}
				}
...
//				      dropList.push(createDrop(x, y));
		    ...
		    wxShare(){
				let self = this;
				this.AppShear = true;
				let Hashs = encodeURIComponent(`#/HouseList/houseDetail?id=${this.id}&productType=${this.productType}&from=xiaochengxu`);
		        ...
		        Wechat.share({
		            message: {
		                ...
		                thumb: "https://img.guoanfamily.com/rent/static/HomePage/chuxin01.png", //图片
		                media: {
		                    "type": 9,
							...
							// url: `/pages/index/index?Hashs=${Hashs}`  //'/pages/rent/rentDetails/rentDetails?id=' + $id + '&productType=' + $productType
							url:('/pages/rent/rentDetails/rentDetails?id=' + this.id + '&productType=' + this.productType)
						}
		            },
...
//					.map((item,index) =>{
//						if(item.typeCode == "0014001"){
//							item.roomItemName = item.roomItemName +"房间";
//						}
//						return item;
//					})
...
//		                }
		                this.communityList = res.data.communityImages.split(",",4).map((item)=>{
		                	item = !item ||this.concatFileUrl(item,320,240);
							return item;
		                })
					}else{
					...
					communityImages = this.imgUrl[0].img;

					if(this.$wechat.miniProgram){
						let Mself = this
						this.$wechat.miniProgram.getEnv((response)=> {
							if(response.miniprogram){
							Mself.$wechat.miniProgram.postMessage({data:{
								shearData:{
									title:Mself.areaName+" "+Mself.houseName+" "+Mself.roomNumber,
									imgurl:communityImages,
									// path:encodeURIComponent(`${Path}?from=xiaochengxu&Hashs=${Hashs}`),

									path: encodeURIComponent(
									`#/HouseList/houseDetail?id=${Mself.id}&productType=${Mself.productType}&from=xiaochengxu`
									),
									tyle:true
								}}})
							}
     	...
     	wxconfig() {
            ...
            wechatStationShare() {
                ...
                this.$wechat.onMenuShareAppMessage({
                    ...
                    imgUrl: "https://img.guoanfamily.com/rent/static/HomePage/chuxin01.png", //图片
                    trigger: (res) => {
				...
				if(!this.LoToken() && !this.UserPhone()){
					...
					if(this.hasDepositContract){
						...
						let token = this.getStorage('token');
				  		if(platform == 'xiaochengxu'){
				  			this.$wechat.miniProgram.navigateTo({url:'/pages/depositReservation/depositReservation?houseId='+this.houseId+'&roomId='+this.roomId+'&token='+token});
				  		}else{
