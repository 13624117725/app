...
<template>
  <div  class="fixContent housingPriceInfo" >
    ...
    <div class="info">
        ...
        <div class="twelve colorNine">楼盘均价：<span class="numbers">{{ averagePrice }}元/㎡</span></div>
        ...
        <div class="Tels">
          {{ consult }}
          <a class="phone" :href="'tel:' + consult"><img src="../../../static/new/img/redTel.png" alt="" style="width:0.8rem"></a>
        </div>
        <div class="telInfo">
          最新政策，更多优惠信息，请致电展示中心
        </div>
    ...
    <div class="priceTrend">
        ...
        <div class="twelve colorThree chouseBtn">
            <span :class="recentYears==1 ? 'nande' : 'nvde'"
                @click="yearOne">
                近1年
            </span>
            <span :class="recentYears==3 ? 'nande' : 'nvde'"
                @click="yearThree">
                近3年
            </span>
        </div>
...
<script>
// 加上标签
import echarts from '../../script/myEcharts'
export default {
...
//        threeYearMax + toDecimal2(Math.abs(threeYearMax - yMaxLength3 * 3)); // 这列也加上了绝对值,暂时无错
      ...
      this.ecOption1 = {
        ...
        var myChart = echarts.init(document.getElementById("ecBox"));
        myChart.setOption(this.ecOption);

        var myChart1 =echarts.init(document.getElementById("ecBox1"));
    ...
    setupdata:function(){
    	var myChart = echarts.init(document.getElementById("ecBox"));
	    	...
	    	this.ecOption.baseOption.dataZoom[0].start = this.ecOption.baseOption.dataZoom[0].start+0;
	    	this.ecOption.baseOption.dataZoom[0].end = this.ecOption.baseOption.dataZoom[0].end+0
	    	myChart.setOption(this.ecOption)
    }
  }
};
