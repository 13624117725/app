    ...
    <div  class="fixContent" >
       <view-box ref="viewBox">
        ...
        <swiper :options="swiperOption" ref="mySwiper" v-if="demo06_list.length > 0">
            <swiper-slide :key='index' v-for="(item,index) in demo06_list"><img style="margin:0 auto;width:6.9rem; height:5.17rem"  @click.stop="show(index)" :src="item.img"  /></swiper-slide>
        ...
        <div class="typeInfo">
            ...
            <div class="detial">
                ...
                <span class="ovaspan">：</span>
                <div class="rightContent">
                    <span class="priceContent">{{typeInformation.internalarea}}</span>
        ...
        <!-- ---------------底部部分------------------------- -->
            <div class="bottom ipxButton" slot="bottom">
            ...
            this.get('palmsaleapp/api/v1/buildtype/findTypeinfoId?typeid=' + this.houseID,{ interfaceType: "newHouse"}).then(response => {
                ...
                if (response.status == 200) {
                    response.data.forEach(item => {
                        let M2index = item.salearea.indexOf("㎡")
                        if(M2index!=-1){
                             item.salearea = item.salearea.split("㎡")[0]
                        }
                    });
                    this.typeInformation = response.data[0];
                    if(this.typeInformation.salearea.indexOf("m²")!=-1){
                        this.typeInformation.salearea = this.typeInformation.salearea.substring(0, this.typeInformation.salearea.length-2)
                    }

                    this.baseList = this.typeInformation.housetypefilename.split(',');
                    this.bannerLen = this.baseList.length;
                    var arr = [];
                    // let ImgObj = {};  如果将对象作为array的一项 push 就要将对象写进循环内
