<template>
    <div class="fixContent" >
      <!-- <div :class="{'topSwiper':IosTop,'topSwiper1':AndroidTop,'topSwiper2':WxTop}"> -->
        <div class="arealist" >
          <div :class="{'topSwiper':IosTop,'topSwiper1':AndroidTop,'topSwiper2':WxTop}"></div>
          <div class="arealist-back" @click="back">
            <span class="back"></span>
          </div>
            <div  class="areatabs" style="margin-top:0 auto;">
              <div class="redbor"></div>
                ...
                <div style="padding-left: 0.45rem;">
                    ...
                    <swiper :options="swiperOption" ref="mySwiper">
                        <swiper-slide @click.native='slideImgClickFn(item)' v-for="(item, i) in buildList" :key="i">
                            <div class="swipediv" style="width:5.48rem;height:4.1rem;">
                                <div class="swipeimg" @click="handleCLickSwipeImg(item)">
                                     <img style="width:100%;height:100%;border-radius:.1rem;" :src='`${item.firstpicture}?imageView2/0/w/350/h/200`'  alt=""/>
                                </div>
...
export default {
  ...
  methods: {
        ...
        if (response.status == 200) {
          ...
          for (let i = 0; i < response.data.length; i++) {
            ...
            if (response.data[i].buildnamepy) {
              response.data[i].buildnamepy = response.data[
                i
              ].buildnamepy.toUpperCase();
            }
            // 筛选区域楼盘
            // console.log(response.data[i].city, 7888)
            var province = response.data[i].province
            if(response.data[i].isAroundBeijing == '1') {
    ...
    // 跳转到楼盘详情
    handleCLickSwipeImg(item) {
      this.$router.push({
        path: "building_detial",
        query: { buildID: item.id }
      });
    },
  ...
  mounted() {
    this.isTypePhone();
    // 跳转到指定的位置
    this.swiper.slideTo(this.$route.query.index, 1000, false);
    this.swipeIndex = this.$route.query.index
  },
  created () {
    this.initLoad();
  },
...
.buildName {
  ...
  span {
    ...
    height: 0.2rem;
    margin-left: 0.15rem;
    background: url("../../../static/new/img/areaAdd.png") no-repeat right
      center;
    background-size: 100%;
  }
}
