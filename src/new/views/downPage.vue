        ...
        btnPayMent() {
            ...
            this.post('palmsaleapp/api/v1/custom/initiation/saveInitCust', post_data,{ interfaceType: "newHouse"}).then(response => {
                ...
                if (response.status == 200) {
                    ...
                    window.location.href = 'https://www.guoanfamily.com/commonPlatform/pay/membershipPayment.html?time='+((new Date()).getTime())+'&id=' + this.openid + '&notifyUrl=' + this.notifyUrl + '&totalFee=' + this.totalFee + '&orderid=' + this.orderid + '&body=' + encodeURIComponent('下定金额') + '&callBackUrl=' + this.callBackUrl;
                }
            }, response => {
...
<style lang="less">
.topBuildInfor .weui-label::before {
    content: "*";
    position: absolute;
