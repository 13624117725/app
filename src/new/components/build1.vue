        ...
        <li class="listoLi" :key="index" v-for="(item,index) in list" v-on:click="buildDetial(item.id)">
          ...
          <div class="listRightDiv">
            ...
            <div class="listaverage" :class="{priceMargin: fjStatus}">
              <span v-if="item.averageprice!=='售价待定'">均价</span>&nbsp;<span style="font-size:0.36rem;font-weight: bold;">{{item.averageprice}}</span><span v-if="item.averageprice!=='售价待定'">元/㎡</span>
            </div>
          </div>
        </li>
        <div class="clear"></div>
        <div style="height:3rem;widht:100%"></div>
      </ul>
